 ...
 * @remarks 对齐工作台：选择标准组、配置规则并发起对齐任务。
 *
 * 规范说明：
...
// -----------------------------------------------------------------------------
// 分段：Alignment.tsx 核心业务逻辑区（状态管理、事件处理与 API 调用）
...
/**
 * 类型定义 `AlignmentTaskDetailData`：描述前后端交互或页面状态结构。
 */
...
/**
 * 函数 `sleep`：本模块内部业务辅助逻辑。
 */
const sleep = (ms: number): Promise<void> => new Promise((resolve) => window.setTimeout(resolve, ms));

/**
 * 函数 `vectorRowToStandard`：本模块内部业务辅助逻辑。
 */
function vectorRowToStandard(row: VectorSourceRow): Standard {
  /**
   * 函数 `source`：本模块内部业务辅助逻辑。
   */
...
/**
 * 异步函数 `fetchAllStandardsPages`：发起 API 请求或执行页面侧异步流程。
 */
...
/**
 * 异步函数 `fetchVectorSourceRows`：发起 API 请求或执行页面侧异步流程。
 */
...
/**
 * 异步函数 `loadStandardsForAlignment`：发起 API 请求或执行页面侧异步流程。
 */
